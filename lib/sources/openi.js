...
export async function searchOpenI(query, limit = 8) {
  ...
  // Open-i sometimes returns HTML (anti-bot page, 5xx error page) instead of
  // JSON, especially from non-browser fetch clients like Vercel Edge. Send a
  // sane User-Agent and parse defensively — return [] on any non-JSON body
  // rather than throwing through to the route's safe() wrapper.
  const r = await fetch(url.toString(), {
    headers: {
      'Accept': 'application/json',
      'User-Agent': 'In-Consult/1.0 (clinical reference; +https://github.com/A-SI-Ben/in-consult-demo)',
    },
  });
  if (!r.ok) return [];
  ...
  if (!trimmed.startsWith('{') && !trimmed.startsWith('[')) {
    console.warn('open-i returned non-JSON (likely HTML error page); skipping');
    return [];
  }
  let data;
  try {
    ...
    console.warn('open-i JSON parse failed; skipping');
    return [];
  }
